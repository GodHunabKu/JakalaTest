					<ul class="list-group">
						...
						<li class="list-group-item">
											<?php if(is_loggedin()) { ?>
											<button type="button" class="btn btn-primary btn-block btn-lg<?php if(is_coins(0)<$total) print ' disabled'; ?>" onclick="$('#buyModal').modal('show');" style="font-size: 1.2rem; padding: 15px;">
												<i class="fa fa-shopping-cart"></i> 
												<?php
													print $lang_shop['buy'].' ';
													if($item[0]['discount']>0)
														print '<span class="current-price">'.$total.' MD</span> <span class="price-strikethrough">'.$price1.' MD</span>';
													else print '<span class="current-price">'.$price1.' MD</span>';
													?>
											</button>
											<?php } if(!is_loggedin()) { ?>
